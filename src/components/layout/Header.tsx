...
export function Header({ onNavigate, onOpenCommand }: HeaderProps) {
  ...
  const allStores = isDemoMode
    ...
    : stores;
  const currentStore =
    allStores.find((s) => s.id === currentStoreId) ?? allStores[0];
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "hidden md:flex items-center gap-1.5 text-sm text-slate-500 flex-shrink-0",
          isRTL && "flex-row-reverse",
        )}
      >
        ...
        <span className="text-slate-700">
          {breadcrumbMap[activeSection] ?? activeSection}
        </span>
      </div>
      ...
      >
        ...
        <div className="relative hidden sm:block">
          ...
          {storeMenuOpen && (
            <>
              ...
              <div
                className={cn(
                  "absolute top-9 z-20 bg-white border border-slate-200 rounded-lg shadow-lg py-1 min-w-[200px]",
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-2 h-8 px-2 rounded-md text-slate-600 hover:bg-slate-100 transition-colors",
            isRTL && "flex-row-reverse",
          )}
        >
